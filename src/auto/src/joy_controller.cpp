...
void joyCb(const sensor_msgs::Joy &joy_msg) {
    ...
    if (joy_msg.buttons[ENABLE_BUTTON]) {
        ...
        radius_x  = 1.0 / (joy_msg.axes[WX_AXE] * joy_msg.axes[WX_AXE] * copysign(curvature_max, joy_msg.axes[WX_AXE]));
        radius_y  = 1.0 / (-joy_msg.axes[WY_AXE] * joy_msg.axes[WY_AXE] * copysign(curvature_max, joy_msg.axes[WY_AXE]));
        w = joy_msg.axes[WX_AXE] * joy_msg.axes[WX_AXE] * copysign(w_max, joy_msg.axes[WX_AXE]);
        ROS_INFO_STREAM("MANUAL " + mode);

        if (mode == "XVEHICLE") {
            steer.xVehicle(vx, radius_x);
        }
        else if (mode == "YVEHICLE") {
            steer.yVehicle(vy, radius_y);
        }
        else if (mode == "PARALLEL") {
